...
<script>
	$(function() {
		...
		$(".Main-DropDown-Menu a").mouseenter(function(){
			$("#Simple-Explain-Div").css("visibility", "inherit");
			...
			if(name == "Company"){
				$(menuName).text("회사 소개");
				$(menuExplain).text("TRYCOFFEE가 어떤 일을 하며 경영 철학 및 본사 위치를 확인 할 수 있습니다.")
			}
			else if(name == "Ceo"){
				$(menuName).text("CEO 인사말");
				$(menuExplain).text("TryCoffee사 CEO의 간략한 인사말을 볼 수 있습니다.");
			}
			else if(name == "Drink"){
				...
				$(menuExplain).text("매장에서 제공하는 미팅룸을 미리 예약 할 수 있습니다.");
			}
			else if(name == "Recipe"){
				$(menuName).text("Recipe Contest");
				$(menuExplain).text("자신만의 커피 및 음료 레시피를 공유해보세요!");
			}
			else if(name == "Client"){
				$(menuName).text("고객의 소리");
				$(menuExplain).text("매장이나 본사에 대한 운영, 서비스에 대한 솔직한 평가와 바라는 점을 조회 및 작성 할 수 있습니다.")
			}
	...
	<div class="collapse navbar-collapse" id="Main-DropDown-Nav">
		...
		<nav class="navbar navbar-default navbar-fixed-top">
			<a id="Main-Brand-Btn" class="navbar-brand" href="front?cmd=main">Try
				Coffee Catch{}<br/>
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<span class="navbar-brand" style="font-size: 11pt">고객 1님</span>
			...
			<ul class="nav navbar-nav" style="margin-left: 8%">
				...
				<li><a id="My-Page-Btn" href="front?cmd=my_page">My Page</a></li>
			...
			<ul class="Main-DropDown-Menu nav navbar-nav" style="width: 100%;">
				...
				<!-- 심플 가이드 -->
				<div id="Simple-Explain-Div" class="col-md-6" style="visibility:hidden; display:inline-block;">
					<h2>
					<i class="fa fa-lightbulb-o" aria-hidden="true" style=" color: yellow; text-shadow: 1px 1px 1px #ccc; font-size: 1.5em;"></i>
					<span style="color:white">심플 가이드</span><hr/>
					</h2>
					<!-- hover 된 dropdown menu명 -->
					<p id="Dropdown-Name" style="color:#18bc9c"></p>
					<!-- 해당 dropdown menu에 대한 설명 -->
					<p id="Dropdown-Simple-Explain" style="color:white"></p><br/>
				</div>
				<!-- dropdown menu -->
